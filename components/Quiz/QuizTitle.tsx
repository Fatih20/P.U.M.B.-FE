...
export default function QuizTitleForm({text, editable}:{text:string,editable:boolean}) {
    // Initiate Router
    ...
    return (
        <>
            {/* Title */}
            {title.visibility &&
                <h1 className="text-2xl text-bold" onClick={() => transformToForm()}>{text}</h1>
            }

            {/* Form */}
            {form.visibility &&
                <form className="rounded bg-white" onSubmit={handleSubmit(handleTitleSubmit)}>
                    <div className="mb-6">
